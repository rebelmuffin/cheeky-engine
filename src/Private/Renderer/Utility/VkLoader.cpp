...
namespace Renderer::Utils
{
    ...
    )
    {
        ...
        fastgltf::Expected<fastgltf::Asset> parse_result = parser.loadGltf(
            load_result.get(), filePath.parent_path(), gltf_options, fastgltf::Category::Meshes
        );
        ...
        for (const fastgltf::Mesh& mesh : asset.meshes)
        {
            ...
            for (const fastgltf::Primitive& primitive : mesh.primitives)
            {
                ...
                {
                    const fastgltf::Accessor& colour_accessor =
                        asset.accessors[colour_attribute->accessorIndex];
